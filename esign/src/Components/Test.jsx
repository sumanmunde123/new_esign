...
// import history from './useCustomHistory';
...
//       pdf.addImage(imgData, "PDF", 0, 0, 210, 297); // Adjust dimensions as needed
...
// const handlePrepareSignRequest = () => {
//   if (htmlCode) {
//     // Use html2canvas to capture the HTML content
//     html2canvas(pdfContainer.current)
//       .then((canvas) => {
//         // Create a new PDF document
//         const pdf = new jsPDF("p", "mm", "a4");

//         // Calculate the height and width based on the content
...
//         // Use history.push to navigate to the Template component and pass the PDF data
//         history.push(`/template/${encodeURIComponent(pdfData)}`);
//       })
//       .catch((error) => {
//         console.error("Error capturing HTML content:", error);
//       });
...
const handlePrepareSignRequest = () => {
    if (htmlCode) {
      html2canvas(pdfContainer.current).then((canvas) => {
        ...
        // Show the Template.js component
...
// convert canvas file into pdf file and  Send file to template page
...
<br></br>
        ...
        <div className="addpdf">
          ...
          {/* <button className="sbtn" onClick={captureCanvasAsPDF}>
          Add Canvas to PDF
        </button> */}
        <button onClick={handlePrepareSignRequest} className="sbtn">
          Prepare Sign Request
        </button>
     
      {showTemplate && <Template pdfData={pdfData} />}
          </div>
